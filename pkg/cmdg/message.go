...
import (
	...
	"github.com/ThomasHabets/cmdg/pkg/cmdg/clib"
...
func (msg *Message) ProcessInlineImages(ctx context.Context, body string, termW, termH int) string {
	...
	for i, img := range images {
		...
		if len(img.Contents) == 0 {
			wg.Add(1)
			go func(idx int, image *InlineImage) {
				defer wg.Done()
				log.Infof("Resolving image %d: %s", idx, image.Source)
				data, err := msg.ResolveCID(ctx, image.Source)
				if err != nil {
					log.Errorf("Failed to resolve image %q: %v", image.Source, err)
					return
				}

				// Use clib to decode and convert to PNG.
				res, ok := clib.DecodeToPNG(data)
				if !ok {
					log.Errorf("Failed to decode image %q with clib", image.Source)
					return
				}
				log.Infof("Image %d converted to PNG, size %dx%d", idx, res.Width, res.Height)

				msg.m.Lock()
				image.Contents = data
				image.PNGContents = res.PNGData
				image.PixelWidth = res.Width
				image.PixelHeight = res.Height
				image.Width, image.Height = calculateCellOccupancy(res.Width, res.Height, termW, termH)

				if proto == "kitty" {
					image.KittyID = allocKittyID()
					log.Infof("Uploading image %d to kitty with ID %d", idx, image.KittyID)
					kittyUploadImage(image.PNGContents, image.KittyID)
				}
				msg.m.Unlock()
			}(i, img)
		}
...
func htmlRender(ctx context.Context, s string, startIdx *int) (string, []*InlineImage, error) {
	elements, ok := clib.HTMLToElements(s)
	if !ok {
		return "", nil, fmt.Errorf("failed to parse HTML with clib")
	}
	...
	for _, elem := range elements {
		log.Debugf("Rendering element type %d: %q", elem.Type, elem.Text)
		switch elem.Type {
		case clib.HElemText:
		...
		case clib.HElemLink:
			out.WriteString(elem.Text)
			if elem.Attr1 != "" && !strings.Contains(elem.Text, elem.Attr1) {
				out.WriteString(" [")
				out.WriteString(elem.Attr1)
				out.WriteString("]")
			}
		case clib.HElemImage:
			...
			*startIdx++
			...
			out.WriteString(fmt.Sprintf(" ##IMG_%d_## ", idx))
	...
	res := out.String()
	log.Infof("Rendered HTML with clib (len %d), found %d images", len(res), len(images))
